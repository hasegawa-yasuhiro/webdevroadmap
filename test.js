    ...
    .then(function (result) {
      ...
      */
      ...
      if(e === "Failed to check remaining tokens less than count") {
        ...
        ***********/
        assert(false, "Bid allowed more tokens than remaining");
      } else {
        /**********
        TASK 3: assert the opposite here
        ***********/
        assert(true, "Bid should revert when more tokens than remaining");
      }
    })
  });
       ...
       */
       ...
       if(e === "Failed to check owner in reveal winners") {
         ...
         HINT:   Use assert(false) to assert false
                 Use assert(true) to assert true
         ***********/
         assert(false, "Non-owner allowed to reveal winners");
       } else {
         /**********
         TASK 6: assert the opposite here
         ***********/
         assert(true, "Reveal winners should revert when called by non-owner");
       }
     })
   })
  ...
  it("Should set winners", function() {
    ...
    ***********/
    ...
    .then(function(result) {
      ...
      ***********/
      assert.notEqual(result, '0x0000000000000000000000000000000000000000', 'Winner address set for item 0');
      /**********
      ...
      ***********/
      ...
      HINT:     Default address is '0x0000000000000000000000000000000000000000'
                ...
                Parameters for notEqual : (result, default address , message);
      ***********/
      assert.notEqual(result, '0x0000000000000000000000000000000000000000', 'Winner address set for item 1');
      /**********
      ...
      ***********/
      ...
      HINT:     Default address is '0x0000000000000000000000000000000000000000'
                ...
                Parameters for notEqual : (result, default address , message);
      ***********/
      assert.notEqual(result, '0x0000000000000000000000000000000000000000', 'Winner address set for item 2');
    })
  });
});
